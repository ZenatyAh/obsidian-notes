...
---
### الفرق بين **المسار المطلق (Absolute)** و **المسار النسبي (Relative)** يشبه الفرق بين إعطاء شخص "عنواناً كاملاً" وإعطائه "اتجاهات من مكانه الحالي".
...
دعنا نستكشف دالة `loader` معاً. إنها واحدة من أهم الميزات التي أضيفت في النسخ الحديثة من React Router (v6.4+)، وتغير طريقة تعاملنا مع جلب البيانات (Data Fetching) بشكل جذري.

ببساطة، وظيفة **`loader`** هي جلب البيانات المطلوبة لصفحة (Route) معينة **قبل** أن يتم عرض (Render) مكوّنات تلك الصفحة.

تخيل الأمر كالتالي:

في السابق (باستخدام useEffect)، كان الزائر يدخل الصفحة، يرى شاشة تحميل (Loading Spinner)، ثم تطلب الصفحة البيانات، وعندما تصل البيانات تظهر المحتوى.

أما مع loader، فإن المتصفح يجلب البيانات أولاً، وعندما تصبح جاهزة، يقوم بعرض الصفحة فوراً وهي "ممتلئة" بالبيانات.

هذا النمط يسمى "Render-as-you-fetch" بدلاً من "Fetch-on-render".

كيف تفضل أن نبدأ التعمق في هذا المفهوم؟

1. **التطبيق العملي:** نرى كود بسيط لكيفية كتابة `loader` وربطه بـ Route معين.
    
2. **استلام البيانات:** كيف نستخدم الخطاف (Hook) المسمى `useLoaderData` للوصول للبيانات داخل المكون (Component).
    
3. **لماذا التغيير؟:** نقاش أعمق حول الفرق بينها وبين `useEffect` وكيف تحل مشكلة "الشلال" (Waterfalls).
